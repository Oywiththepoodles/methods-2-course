...
#load data
child_agression_data <- read_delim("ChildAggression.dat", delim = ' ')
```
...
Already now you should try to answer the following questions:
  What is the type of variables you can see in this data? numerical/continuous variables?
...
```{r}
#make a linear model
Agression_television <- lm(Aggression ~ Television, child_agression_data)

#see results of model fit to data
summary(Agression_television)
```
...
```{r}
#plot the data
ggplot(child_agression_data, aes(Television, Aggression)) +  #base layer
  geom_point() + #scatter plot
  ggtitle('Scatter plot over television time and aggresion')
```
...
sum <- summary(Agression_television)
...
ggplot(child_agression_data, aes(Television, Aggression)) + 
  geom_point() + 
...
Diet has the least affect -0.1 (from adjusted r squared)
...
ggplot(child_agression_data, aes(Television, Aggression)) + 
  ...
  ggtitle('Scatter plot over aggression and television')

#Aggression and parenting style:
Agression_Parenting_Style <- lm(Aggression ~ Parenting_Style, child_agression_data)

summary(Agression_Parenting_Style)

ggplot(child_agression_data, aes(Parenting_Style, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se = FALSE) +
  ggtitle('Scatter plot over aggression and parenting style')

#Aggression and siblings:
Agression_Sibling_Aggression <- lm(Aggression ~ Sibling_Aggression, child_agression_data)

summary(Agression_Sibling_Aggression)

ggplot(child_agression_data, aes(Sibling_Aggression, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se = FALSE) +
  ggtitle('Scatter plot over aggression and sibling aggression')

#Aggresion and computergames:
Agression_Computer_Games <- lm(Aggression ~ Computer_Games, child_agression_data)

summary(Agression_Computer_Games)

ggplot(child_agression_data, aes(Computer_Games, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se = FALSE) +
  ggtitle('Scatter plot over aggression and Computer_Games')

#Aggression and diet:
Agression_Diet <- lm(Aggression ~ Diet, child_agression_data)

summary(Agression_Diet)

ggplot(child_agression_data, aes(Diet, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se = FALSE) +
  ggtitle('Scatter plot over aggression and diet')
```
...
```{r}
mm <- lm(Aggression ~ Computer_Games + Television, child_agression_data)
summary(mm)
```
...
```{r}
#check cor()
cor(child_agression_data)

#same, but rounded to 2 decimal points
round(cor(child_agression_data),2) #much better! but still hard to see
```
...
#plot as heatmap
qplot(x=Var1, y=Var2, 
      data=melt(cor(child_agression_data, use="p")), 
      fill=value, 
      geom="tile") +
...
plot_df <- melt(cor(child_agression_data))
...
```{r}
#1) where aggression is predicted by just the predictor you picked (Parenting style)
Agression_Parenting_Style <- lm(Aggression ~ Parenting_Style, child_agression_data)
...
#2) where aggression is predicted by your predictor and some another additional predictor from the dataset
Aggression_Parent_Sibling<-lm(Aggression~Parenting_Style+Sibling_Aggression,child_agression_data)

summary(Aggression_Parent_Sibling)

#3) where aggression is predicted by your predictor and two additional predictors from the dataset
Aggression_Parent_Sibling_Computer_Games<-lm(Aggression~Parenting_Style+Sibling_Aggression+Computer_Games,child_agression_data)

summary(Aggression_Parent_Sibling_Computer_Games)
```
